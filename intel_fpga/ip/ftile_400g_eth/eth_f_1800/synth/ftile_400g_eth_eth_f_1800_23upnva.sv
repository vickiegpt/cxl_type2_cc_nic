...
//(* tile_ip,tile_type="f_tile" *) Tag not used anymore
  ...
  module ftile_400g_eth_eth_f_1800_23upnva
  #(
    ...
    parameter           NUM_CLKS                                                = 1, // Valid values are { 1, 2, 4}
    ...
    parameter           NUM_100GE_PORT                                          = 0,
    parameter           NUM_50GE_PORT                                           = 0,
    parameter           NUM_25GE_PORT                                           = 0,
    parameter           RCFG_GRP                                                = "", // Internal use only, user please DO NOT MODIFY
    ...
    parameter           PKT_CYL                                                 = EHIP_RATE=="400G" ? 2 : 1, // Internal use only, user please DO NOT MODIFY
    ...
    parameter          bb_f_ehip_mac_forward_rx_pause_requests_int              = "DISABLE",
...
(* tile_ip_find_net *)    input  wire             i_clk_sys, // Ethernet system clock
   ...
   // SIP ports
    ...
    output wire                                   o_clk_tx_div, //  this is /66 clock (390.625 MHz) EHIP system clock times 64/66
    output wire                                   o_clk_rec_div64, // this is /64 clock (402.83 MHz), receovered clock
    output wire                                   o_clk_rec_div, // this is /66 clock (390.625 MHz), receovered clock
    ...
    output wire                                   o_cdr_lock, //Signal to indicate that the PMA has locked to data.
    output wire                                   o_tx_pll_locked, //Signal to indicate that the TX PLL has locked.
    output wire                                   o_sys_pll_locked, //Signal to indicate that the o_clk_pll is good to use
...
// PTP auto connection
  ...
  //  wire  [LANE_NUM-1:0]                          pld_pll_cal_done;
   ...
   wire  [AIB_LANES-1:0]                         ptp_placement_virtual;
    ...
    // MAC bb ports
    wire [80*AIB_LANES-1:0]                      tx_parallel_data;
    wire [80*AIB_LANES-1:0]                      rx_parallel_data;
    wire [32*AIB_LANES-1:0]                      hdpldadapt_out_link;
    wire [40*AIB_LANES-1:0]                      hdpldadapt_in_link;
...
generate
    ...
    else if(ENABLE_ASYNC_ADAPTERS == 1) begin
...
(* tile_ip_sip *)   eth_f_sip_23upnva #(
        ...
        .TX_EHIP_PP_EN                            (TX_EHIP_PP_EN                 ),
        .READY_LATENCY                            (READY_LATENCY                 ),
        .ENABLE_ASYNC_ADAPTERS                    (ENABLE_ASYNC_ADAPTERS         ),
        .bb_f_ehip_mac_forward_rx_pause_requests  (bb_f_ehip_mac_forward_rx_pause_requests),
        .bb_f_ehip_mac_flow_control_sip           (bb_f_ehip_mac_flow_control_sip    ),
        .bb_f_ehip_mac_rx_en_pp                   (bb_f_ehip_mac_rx_preamble_passthrough),
        ...
        .RSFEC_TYPE                               (RSFEC_TYPE                    ),
        .ENABLE_ADME                              (ENABLE_ADME                   ),
        .ENABLE_ETK                               (ENABLE_ETK                    ),
	.ENABLE_SOFT_CWBIN                        (ENABLE_SOFT_CWBIN              ),
        .BASE_SEC_ENABLE			  (BASE_SEC_ENABLE		),
	.AVMM2_ADDR_WIDTH			  (AVMM2_ADDR_WIDTH		),
  ...
  // Signals from AVMM1 building block
        ...
        .pld_avmm1_reserved_out                   (pld_avmm1_reserved_out[2:0]   ), // 3 bits
...
(* tile_ip_hip *)  eth_f_hip_23upnva #(
        ...
        .bb_f_ux_rx_fw_clearing_regsiter_attr                     (bb_f_ux_rx_fw_clearing_regsiter_attr),
        .bb_f_ux_rx_bypass_on_chip_ac_cap                         (bb_f_ux_rx_bypass_on_chip_ac_cap),
        .bb_f_ux_rx_vcm_greater_than_0p35v_with_bypass_on_chip_ac_cap (bb_f_ux_rx_vcm_greater_than_0p35v_with_bypass_on_chip_ac_cap),
