...
module eth_f_packet_client_top #(
        ...
        output  logic [WORDS-1:0]     o_tx_mac_inframe,
        output  logic [WORDS*3-1:0]   o_tx_mac_eop_empty,
        output  logic [WORDS*64-1:0]  o_tx_mac_data,
        output  logic [WORDS-1:0]     o_tx_mac_error,
        output  logic [WORDS-1:0]     o_tx_mac_skip_crc,

        input   logic                 i_rx_mac_valid,
        input   logic [WORDS-1:0]     i_rx_mac_inframe,
        input   logic [WORDS*3-1:0]   i_rx_mac_eop_empty,
        input   logic [WORDS*64-1:0]  i_rx_mac_data,
        input   logic [2*WORDS-1:0]   i_rx_mac_error,
        input   logic [WORDS-1:0]     i_rx_mac_fcs_error,
        input   logic [3*WORDS-1:0]   i_rx_mac_status,
...
localparam CTRL_BCNT = (CLIENT_IF_TYPE==0) ? (WORDS+1) : 2;
...
logic [48:0]           cfg_rom_dest_addr_sync, cfg_rom_dest_addr;
logic [31:0]           cfg_rom_crc_pktgap_sync, cfg_rom_crc_pktgap;
...
eth_f_loopback_client #(
        ...
        .cfg_rom_crc_ena        (cfg_rom_crc_pktgap_sync[16])
);
defparam loopback_client.INIT_FILE_DATA  = PKT_ROM_INIT_FILE;
defparam loopback_client.ENA_CRC_GAP_DA = ENA_CRC_GAP_DA;
...
eth_f_packet_client_tx_interface packet_client_tx_if (
        ...
        .cfg_rom_dest_addr       (cfg_rom_dest_addr_sync),
        .cfg_rom_crc_pktgap      (cfg_rom_crc_pktgap_sync)
);
...
defparam packet_client_tx_if.ENA_CRC_GAP_DA     = ENA_CRC_GAP_DA;
...
eth_f_packet_client_csr packet_client_csr (
        ...
        .cfg_rom_dest_addr       (cfg_rom_dest_addr),
        .cfg_rom_crc_pktgap      (cfg_rom_crc_pktgap),
...
eth_f_multibit_sync #(
    .WIDTH(49)
) cfg_rom_da_sync_inst (
    .clk (i_clk_tx),
    .reset_n (1'b1),
    .din (cfg_rom_dest_addr),
    .dout (cfg_rom_dest_addr_sync)
);

eth_f_multibit_sync #(
    .WIDTH(32)
) cfg_rom_crc_pktgap_sync_inst (
    .clk (i_clk_tx),
    .reset_n (1'b1),
    .din (cfg_rom_crc_pktgap),
    .dout (cfg_rom_crc_pktgap_sync)
);


eth_f_multibit_sync #(
    .WIDTH(ROM_ADDR_WIDTH)
) cfg_rom_start_addr_sync_inst (
    .clk (i_clk_tx),
    .reset_n (1'b1),
    .din (cfg_rom_start_addr),
    .dout (cfg_rom_start_addr_sync)
);
eth_f_multibit_sync #(
    .WIDTH(ROM_ADDR_WIDTH)
) cfg_rom_end_addr_sync_inst (
    .clk (i_clk_tx),
    .reset_n (1'b1),
    .din ( cfg_rom_end_addr),
    .dout (cfg_rom_end_addr_sync)
);
eth_f_multibit_sync #(
    .WIDTH(ROM_ADDR_WIDTH)
) cfg_test_loop_cnt_sync_inst (
    .clk (i_clk_tx),
    .reset_n (1'b1),
...
//);
   ...
   eth_f_xcvr_resync_std #(
        ...
        .WIDTH              (1),
        .INIT_VALUE         (0)
    ) cfg_pkt_gen_tx_en_sync (
        .clk                (i_clk_tx),
        .reset              (1'b0),
        .d                  (cfg_pkt_client_ctrl[0]),
        .q                  (cfg_pkt_gen_tx_en)
    ); 
//assign cfg_pkt_gen_tx_en        = cfg_pkt_client_ctrl_sync[0];
  eth_f_xcvr_resync_std #(
        ...
        .WIDTH              (1),
        .INIT_VALUE         (0)
    ) cfg_pkt_gen_cont_mode_sync (
        .clk                (i_clk_tx),
        .reset              (1'b0),
        .d                  (cfg_pkt_client_ctrl[2]),
        .q                  (cfg_pkt_gen_cont_mode)
    ); 
//assign cfg_pkt_gen_cont_mode =  cfg_pkt_client_ctrl_sync[2];
  eth_f_xcvr_resync_std #(
        ...
        .WIDTH              (1),
        .INIT_VALUE         (0)
    ) cfg_tx_select_pkt_gen_sync (
        .clk                (i_clk_tx),
        .reset              (1'b0),
        .d                  (!cfg_pkt_client_ctrl[4]),
        .q                  (cfg_tx_select_pkt_gen)
    );
//assign cfg_tx_select_pkt_gen    = !cfg_pkt_client_ctrl_sync[4]; //Loopback Enable bit 
 eth_f_xcvr_resync_std #(
        ...
        .WIDTH              (1),
        .INIT_VALUE         (0)
    ) stat_cntr_snapshot_sync (
        .clk                (i_clk_tx),
        .reset              (1'b0),
        .d                  (cfg_pkt_client_ctrl[6]),
        .q                  (stat_cntr_snapshot)
    );
//assign stat_cntr_snapshot = cfg_pkt_client_ctrl_sync[6];
 eth_f_xcvr_resync_std #(
        ...
        .WIDTH              (1),
        .INIT_VALUE         (0)
    ) stat_cntr_clear_sync (
        .clk                (i_clk_tx),
        .reset              (1'b0),
        .d                  (cfg_pkt_client_ctrl[7]),
        .q                  (stat_cntr_clear)
    );
