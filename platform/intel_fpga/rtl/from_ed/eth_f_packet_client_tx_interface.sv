...
module eth_f_packet_client_tx_interface #(
        ...
        output logic [WORDS-1:0]             tx_mac_skip_crc,
        ...
        input logic  [48:0]   cfg_rom_dest_addr,
        input logic  [31:0]   cfg_rom_crc_pktgap
);

//---------------------------------------------
logic data_valid, tx_wr;
logic tx_vld, tx_vld_avst, tx_vld_seg, tx_mac_ready_t, tx_mac_valid_t, tx_valid_t, tx_ready_t ;
logic tx_en_reg;
...
assign data_req     = (tx_vld | !data_valid) & tx_en_reg;
assign tx_wr        = din_vld & (!data_valid | tx_vld) & tx_en_reg; 
assign tx_valid_t     = data_valid & tx_ready_t;
assign tx_mac_valid_t = data_valid & tx_mac_ready_t;
...
always @ (posedge clk) begin
    if (rst)              tx_en_reg <= 1'b0;
    else 						tx_en_reg <= tx_en;
end
...
generate 
  if (CLIENT_IF_TYPE == 0) begin:SEG 
    ...
    logic [WORDS*64-1:0] tx_mac_data_t;
    logic [WORDS-1:0] tx_mac_error_t, tx_mac_skip_crc_t, tx_mac_inframe_t;
    logic [WORDS*3-1:0] tx_mac_eop_empty_t;
    logic tx_mac_sop, tx_mac_eop;


    if (ENABLE_PTP == 0 & ENA_CRC_GAP_DA == 1) begin : SEG_GAP_DA_CRC_GEN

    logic ready_en;
    logic [4:0] ready_cnt;

    eth_f_seg_crc_da_tx_cal #(
        .NUM_SEG(WORDS)
    ) crc32_cal_da (
        .clk(clk),
        .rst(rst),
        .ena(cfg_rom_crc_pktgap[16]),
        .i_data(tx_mac_data_t),
        .i_empty(tx_mac_eop_empty_t),
        .i_inframe(tx_mac_inframe_t),
        .i_vld(tx_vld_seg),
        .i_ready(tx_mac_ready),
        .i_skip_crc(tx_mac_skip_crc_t),
        .i_error(tx_mac_error_t),
        .i_cfg_rom_dest_addr(cfg_rom_dest_addr),
        .o_data(tx_mac_data),
        .o_inframe(tx_mac_inframe),
        .o_error(tx_mac_error),
        .o_empty(tx_mac_eop_empty),
        .o_vld(tx_mac_valid),
        .o_ready(tx_mac_ready_t),
        .o_skip_crc(tx_mac_skip_crc)
    );

// Insert packet/cycle gap
    always_ff @(posedge clk) begin
        if (rst) begin
            ready_cnt           <= '0;
            ready_en            <= '0;
        end 
        else if (tx_mac_ready_t) begin
            if (tx_mac_eop & ready_en) begin
                ready_cnt           <= 1'b1;
                ready_en            <= '0 ;
            end
            else if (!(|(ready_cnt ^ cfg_rom_crc_pktgap[4:0]))) ready_en <= 1'b1;
            else ready_cnt <= ready_cnt + 1'b1;
        end
    end

    assign tx_vld_seg = (|cfg_rom_crc_pktgap[4:0])? (tx_mac_valid_t & ready_en) : tx_mac_valid_t;

    end else begin : SEG_PTP_NONCRC
    assign tx_mac_inframe   = tx_mac_inframe_t;
    assign tx_mac_skip_crc  = tx_mac_skip_crc_t;
    assign tx_mac_eop_empty = tx_mac_eop_empty_t;
    assign tx_mac_error     = tx_mac_error_t;
    assign tx_mac_data      = tx_mac_data_t;
    assign tx_mac_ready_t   = tx_mac_ready;
    assign tx_mac_valid     = tx_vld_seg;
    assign tx_vld_seg       = tx_mac_ready_t & tx_mac_valid_t;
    end

    logic [WORDS-1:0]             tx_mac_inframe_r;

    assign tx_mac_inframe_t = tx_vld_seg ? tx_mac_inframe_r : {WORDS{1'b0}};
    ...
    always @ (posedge clk) begin
        if (rst) begin
                ...
                tx_mac_skip_crc_t   <= {WORDS{ctrl_bus[SEG_SKIP_CRC_POS]}};
                tx_mac_inframe_r    <= ctrl_bus[SEG_INFRAME_POS+SEG_INFRAME_WIDTH-1 : SEG_INFRAME_POS];
                tx_mac_eop_empty_t  <= ctrl_bus[SEG_EOP_EMPTY_POS+SEG_EOP_EMPTY_WIDTH-1 : SEG_EOP_EMPTY_POS];
                tx_mac_error_t      <= ctrl_bus[SEG_ERR_POS+SEG_ERR_WIDTH-1 : SEG_ERR_POS];
                tx_mac_data_t       <= data_bus;
        end
    end
    assign tx_sop      = 0;
    assign tx_eop      = 0;
    ...
    logic [WORDS*64-1:0] tx_data_t;
    logic [AVST_EMPTY_WIDTH-1:0] tx_empty_t;
    logic tx_sop_t, tx_eop_t, tx_error_t, tx_skip_crc_t;

    if (ENABLE_PTP == 0 & ENA_CRC_GAP_DA == 1) begin : AvST_GAP_DA_CRC_GEN

    logic ready_en;
    logic [4:0] ready_cnt;

    eth_f_avst_crc_da_tx_cal #(
    .WORDS(WORDS),
    .EMPTY_WIDTH(AVST_EMPTY_WIDTH)
    ) crc32_cal_da (
    .clk(clk),
    .rst(rst),
    .ena(cfg_rom_crc_pktgap[16]),
...
// Insert packet/cycle gap
    always_ff @(posedge clk) begin
        ...
        else if (tx_ready_t) begin
            if (tx_eop_t & ready_en) begin
                ready_cnt           <= 1'b1;
                ready_en            <= '0 ;
            end
            else if (!(|(ready_cnt ^ cfg_rom_crc_pktgap[4:0]))) ready_en <= 1'b1;
    ...
    assign tx_vld_avst = (!(|cfg_rom_crc_pktgap[4:0]))? tx_valid_t : (tx_valid_t & ready_en);
    
    end else begin : AvST_PTP_NONCRC
    assign tx_sop       = tx_sop_t;
    assign tx_eop       = tx_eop_t;
    assign tx_valid     = tx_vld_avst;
    assign tx_error     = tx_error_t;
    assign tx_skip_crc  = tx_skip_crc_t;
    assign tx_ready_t   = tx_ready;
    assign tx_empty     = tx_empty_t;
    assign tx_data      = tx_data_t;
    assign tx_vld_avst  = tx_ready_t & tx_valid_t;
    end

    assign tx_sop_t = tx_vld_avst & tx_sop_r;
    assign tx_eop_t = tx_vld_avst & tx_eop_r;
    ...
    always @ (posedge clk) begin
        if (rst) begin
            ...
            tx_error_t      <= ctrl_bus[AVST_ERR_POS];
            tx_skip_crc_t   <= ctrl_bus[AVST_CRC_POS];
            tx_empty_t      <= ctrl_bus[AVST_EMPTY_POS+AVST_EMPTY_WIDTH-1 : AVST_EMPTY_POS];
            tx_data_t       <= data_bus;
        end
    end
    ...
    assign tx_vld_seg       = 0;
    assign tx_mac_valid     = 0;
    assign tx_mac_ready_t   = 0;
...
wire [2*WORDS-1:0] tx_mac_error_exp; //tx_mac_error expand double to match rx_mac_error

genvar i;
generate
    for (i=0;i<WORDS;i=i+1) begin: mac_error_expand
